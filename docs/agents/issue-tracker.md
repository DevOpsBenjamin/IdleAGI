...
- **Créer une branche par ticket** : `git checkout -b <type>/issue-<n>-<slug>` (ex: `feat/issue-2-tokenizer-core`, `docs/issue-3-adr-cooling`).
...
- **Child ticket**: an issue linked to the map as a child ticket (listed in the map issue body with `Part of #<map>`). Labels: `wayfinder:<type>` (`research`/`prototype`/`grilling`/`task`).
- **Blocking**: GitHub's native issue dependencies or fallback `Blocked by: #<n>`.
