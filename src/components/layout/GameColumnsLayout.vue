...
import { ref } from 'vue'
import type { MobileTab } from '@/components/MobileNavigation.vue'
...
defineProps<{
  ...
  hasHardware: boolean
  hasCpu: boolean
  hardwareArray: HardwareNode[]
  upgradesArray: SoftwareUpgrade[]
  ramUpgradesArray: SoftwareUpgrade[]
  coolingUpgradesArray: SoftwareUpgrade[]
  powerUpgradesArray: SoftwareUpgrade[]
...
<template>
  <main class="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 pb-24 lg:pb-6 min-h-0 z-10 font-mono">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 max-w-7xl mx-auto w-full">
      ...
      >
        ...
        <!-- 5. Tri-Allocation Panel (Unlocked in Phase 3) -->
        <Transition name="fade-slide">
          <AllocationPanel
            v-if="store.unlockedFeatures.trainingAllocation"
